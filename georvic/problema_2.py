...
while (horas < max_horas_2) :
    ...
    if (cliente < len(llegadas)) and (llegadas[cliente] <= horas) :
        ...
        # Encontramos la cola más corta
        cola_mas_corta = np.argmin([len(cajeros[i].cola) for i in range(4)])
        
        cola_clientes = len(cajeros[cola_mas_corta].cola)
        ...
        if (not cliente_actual_declina) :
            ...
            cajeros[cola_mas_corta].cola.append(cliente)
            clientes_que_se_quedan += 1
            print("LLegada de un cliente al cajero {} con {} clientes en espera. Tiempo (minutos) {}"\
                    .format(cola_clientes, cola_mas_corta, horas))
    ...
    for i in range(4) :
        if (len(cajeros[i].cola) > 0) and cajeros[i].disponible :
            ...
            #cola_clientes -= 1
            print("Cajero {} atiende a un cliente. Su cola es de {} clientes. Tiempo (min) : {}"\
                    .format(i, len(cajeros[i].cola), horas))
    ...
    for i in range(4) :
        if (not cajeros[i].disponible) and ((cajeros[i].inicio + cajeros[i].tiempo_de_atencion) <= horas) :
            ...
            cliente_atendido = cajeros[i].cola.pop(0)
